...
function rubbishMail(mailId){
	var flag  = window.confirm("确定将该邮件放入垃圾箱");
	if (flag == true){
		var deleteURL = "<%=path%>/MailAction!rubbish?mailId="+mailId+"";
		window.location.href = deleteURL;
	}
}
</script>
		...
		<s:iterator value="mailList" status="myStatus">
			<tr>
				...
				<td>
					...
					<a class="btn btn-warning btn-xs" href="javascript:void(0);" onclick="rubbishMail('<s:property value="mailId"/>');">放入垃圾箱</a>
