...
<script language="javascript">
	...
	function deleteOptions(options,surveyId){
		var flag  = window.confirm("确定删除该选项");
		if (flag == true){ 
			var deleteURL = "<%=path%>/SurveyAction!deleteOptions?options="
	...
	<table class="table table-bordered" width="100%" border="1"
		...
		<tr>
			...
			<td><s:textfield class="form-control selectpicker"
					style="width:30%" name="surveyTitle" /> <s:hidden name="surveyId" />
				<s:hidden name="beginTime" /> <s:hidden name="endTime" /></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td id="d">
				<p>原有选项:</p>
				<s:iterator value="options" status="status" id="it">
					<p>
						标题
						<s:textfield name="options" value="%{options[#status.index]}" />
						<a class="btn btn-danger btn-xs" href="javascript:void(0);"
							onclick="deleteOptions('<s:property value="#it"/>','<s:property value="surveyId"/>');">删除</a>
					</p>
				</s:iterator>
			</td>
